...
export default function CategoryDetail({
  ...
  if (!category || !group) {
    return <div className="p-6">{t('permissions.no_categories', 'No API categories')}</div>;
  }
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
          ...
          <span className="text-xs text-slate-400">{t('permissions.click_to_toggle')}</span>
        </div>
          ...
          <div key={api.id} className="px-5 py-4 border-b border-slate-50 hover:bg-slate-50/60 transition-all">
            <div className="flex items-center gap-3 mb-3">
              <span className={`px-2 py-0.5 rounded text-[9px] font-black text-white ${
                {POST: 'bg-emerald-600', PUT: 'bg-amber-600', PATCH: 'bg-orange-500', GET: 'bg-blue-500', DELETE: 'bg-red-500'}[api.method] || 'bg-slate-500'
              }`}>
                {api.method}
              </span>
            ...
            <div className="flex flex-wrap gap-2 pl-1">
              <span className="text-[9px] text-slate-400 font-semibold self-center mr-1">{t('permissions.allowed')}</span>
              ...
              {activePartners.length === 0 && (
                <span className="text-[10px] text-slate-300 italic">{t('permissions.no_active_partners')}</span>
              )}
            </div>
          </div>
        ))}
